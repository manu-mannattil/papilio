...
- All equations correspond to those in Peatross and Ware (2015) [1].
...
def reflectance_p(lam, n_list, d_list, theta_0=0, n_0=1, n_sub=1):
    ...
    A @= np.array([[n_0, np.cos(theta_0)],
                   [n_0, -np.cos(theta_0)]])
    A @= M
    A @= np.array([[np.cos(theta_sub), 0],
                   [n_sub, 0]])
...
def reflectance(*args, **kwargs):
    """Compute the multilayer reflectance for unpolarized light."""

    return 0.5*(reflectance_s(*args, **kwargs) + reflectance_p(*args, **kwargs))

def reflectance_to_hex(
    lam_list,
    r_list,
    illuminant="D65",
